...
master <- set %>%
         mutate( model = map(data.x, fit_model),
...
select <- master %>% unnest(glance) %>%
                     filter(r.squared >= 0.75) 

master %>% unnest(glance) %>%
           ggplot( mapping = aes(x = Category_ESI_cn, y = r.squared, label = Category_ESI_cn) ) +
           geom_point() +
           geom_text(check_overlap = TRUE, na.rm =TRUE) +
           theme(axis.line=element_blank(),
                 axis.text.x=element_blank(),
                 text=element_text(size=14)) +
           geom_point(data=select, aes(x=Category_ESI_cn, y=r.squared), colour ="red") 
...
```{r}
checkdata <- select %>% unnest(predictions) %>%
                        select(Category_ESI_cn,  data.y, paper, pred) %>%
                        unnest(data.y) 


compare <- checkdata %>% mutate(delta =  abs((paper - pred)/paper) ) %>%
